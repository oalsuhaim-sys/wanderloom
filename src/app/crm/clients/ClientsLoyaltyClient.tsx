...
} from '@/app/crm/clients/useClientDirectory'
...
/** Lightweight row/card placeholders — chrome stays visible, no full-page spinner. */
function ClientsDirectorySkeleton({ viewMode }: { viewMode: 'cards' | 'table' }) {
  if (viewMode === 'table') {
    return (
      <div className={`${CRM_TABLE_SCROLL} overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-[#2D3F3A] dark:bg-[#22302C]`}>
        <div className="space-y-3 p-4">
          {Array.from({ length: 8 }).map((_, i) => (
            <div
              key={i}
              className="h-12 animate-pulse rounded-xl bg-slate-100 dark:bg-[#1A2421]"
  ...
  return (
    <div className={CRM_CARD_GRID}>
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="h-44 animate-pulse rounded-2xl border border-slate-200 bg-slate-100 dark:border-[#2D3F3A] dark:bg-[#1A2421]"
...
export function ClientsLoyaltyClient({
  initialData = null,
}: {
  initialData?: ClientDirectoryPayload | null
}) {
  ...
  const {
    ...
    mutate,
    refresh,
  } = useClientDirectory({ fallbackData: initialData })
  ...
  useEffect(() => {
    if (directoryError) showErrorToast(directoryError)
    // eslint-disable-next-line react-hooks/exhaustive-deps -- toast on error change only
  }, [directoryError])
  ...
  const patchDirectory = useCallback(
    (updater: (prev: ClientDirectoryPayload) => ClientDirectoryPayload) => {
      void mutate(
        (current) => {
          const base: ClientDirectoryPayload = current ?? {
            rows: [],
            tripCounts: {},
            profitTotals: {},
          }
  ...
  const handleDeleteClient = async (client: VipClientProfile) => {
    ...
    try {
      ...
      patchDirectory((prev) => {
        const nextTrip = { ...prev.tripCounts }
        const nextProfit = { ...prev.profitTotals }
        delete nextTrip[client.id]
        delete nextTrip[removedId]
        delete nextProfit[client.id]
        delete nextProfit[removedId]
        return {
          rows: prev.rows.filter(
            (c) => String(c.id) !== removedId && String(c.id) !== String(client.id),
          ),
          tripCounts: nextTrip,
          profitTotals: nextProfit,
        }
      })
        ...
        <header className="flex flex-col gap-4 rounded-2xl bg-slate-900 p-4 text-white shadow-sm sm:flex-row sm:justify-between sm:p-6 dark:border dark:border-[#D4AF37]/30 dark:!bg-[#22302C] dark:text-[#D4AF37]">
          ...
          <div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            <button
              ...
              disabled={showSkeleton || clients.length === 0}
        ...
        <div className="mb-2 w-full">
          ...
          {!showSkeleton ? (
            <p className="mb-4 text-xs text-slate-500 dark:text-slate-400">
              ...
              {isValidating ? ' · جاري التحديث…' : ''}
            </p>
          ) : null}
        </div>

        {showSkeleton ? (
          <ClientsDirectorySkeleton viewMode={viewMode} />
        ) : filtered.length === 0 ? (
              ...
              <div className="space-y-4 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-[#2D3F3A] dark:bg-[#1A2421]/60">
                ...
                <label className="block">
                  ...
                  <input
                    value={form.ref_code}
                    onChange={(e) =>
                      setForm({ ...form, ref_code: e.target.value.toUpperCase() })
                    }
                    className={MODAL_FIELD}
                    dir="ltr"
                    placeholder="WL-…"
