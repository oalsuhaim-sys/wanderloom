...
export type ExpertCommissionProfile = {
  id: string;
  name?: string | null;
...
type Props = {
  open: boolean;
  profile: ExpertCommissionProfile | null;
  apiBase: string;
  onClose: () => void;
...
/**
 * Edit Expert — referral code + commission rate (default 15% of profit margin).
 */
export default function EditExpertModal({
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success('تم حفظ نسبة عمولة الخبير');
      await onSaved?.();
      onClose();
    } catch (err) {
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="edit-expert-commission-title"
    >
      <div
      ...
      >
        <div className="mb-4 flex items-center justify-between gap-3">
          <h2
            id="edit-expert-commission-title"
            className="text-lg font-bold text-slate-900 dark:text-white"
          >
            عمولة الخبير — {profile.name || 'خبير الوجهة'}
          </h2>
          <button
            type="button"
            onClick={onClose}
