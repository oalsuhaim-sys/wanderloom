    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      {hotels.map((hotel) => (
        <article key={hotel.id} className={GLASS_CARD_CLASS}>
          <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#F9F9F6]">
            {hotel.imageUrl ? (
              <>
          ...
          <div className="p-4">
            <h3 className="text-base font-extrabold text-[#1E2720]">{hotel.name}</h3>
            {hotel.city ? <p className="mt-1 text-xs font-medium text-[#1E2720]/55">{hotel.city}</p> : null}
            <dl className="mt-3 space-y-1.5 text-xs">
              ...
              {hotel.checkOut ? (
                <div className="flex justify-between gap-2">
                  <dt className="font-semibold text-[#1E2720]/45">{TXT_CHECKOUT}</dt>
                  <dd className="font-bold text-[#1E2720]/85" dir="ltr">
                    {hotel.checkOut}
                  </dd>
                </div>
              ) : null}
              {hotel.bookingReference ? (
                <div className="flex justify-between gap-2">
                  <dt className="font-semibold text-[#1E2720]/45">{TXT_REF}</dt>
