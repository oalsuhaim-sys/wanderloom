-- VIP Ledger: محفظة العهدة المالية (clients.wallet_balance + wallet_transactions)
-- + Automated VIP tiers (total_spent / vip_tier)
...
comment on column public.clients.wallet_balance is 'رصيد عهدة VIP المتبقي (ر.س)';
...
comment on table public.wallet_transactions is 'سجل إيداع/خصم عهدة VIP للعميل';

create index if not exists wallet_transactions_client_created_idx
...
create policy "wallet_transactions_anon_all"
...
drop policy if exists "wallet_transactions_authenticated_all" on public.wallet_transactions;
