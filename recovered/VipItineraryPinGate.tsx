            ...
            <input
              ...
              onChange={(e) => {
                ...
                const normalized = raw.includes('-')
                  ? raw.toUpperCase().replace(/[^A-Z0-9-]/g, '').slice(0, 24)
                  : raw.replace(/\D/g, '').slice(0, 8)
                onPinChange(normalized)
              }}
              placeholder="•••• أو WL-XXXX-XX"
              className="w-full rounded-xl border border-[#D4AF37]/35 bg-[#2A362C]/60 py-4 pe-4 ps-11 text-center text-xl font-semibold tracking-[0.15em] text-[#E5C158] outline-none backdrop-blur-sm transition placeholder:text-[#D4AF37]/30 focus:border-[#E5C158] focus:ring-1 focus:ring-[#E5C158]/45"
              required
            />
          </div>
